...
- **Details**: Added Vercel serverless route file `api/[...path].ts` and `vercel.json` rewrites for core public/auth bootstrap endpoints. Remaining API surface still lives in `server.ts` and should be migrated incrementally if full parity is required.
